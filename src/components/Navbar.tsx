  ...
  return <motion.nav initial={{
    ...
    ease: "easeOut"
      ...
      <div className="container mx-auto px-6 flex items-center justify-between">
        ...
        {/* Mobile Menu Button */}
        <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="md:hidden text-foreground p-2" aria-label="Menu">
      ...
      <AnimatePresence>
        ...
        height: 0
            ...
            <div className="flex flex-col p-6 gap-4">
              {navLinks.map(link => <a key={link.label} href={link.href} onClick={() => setIsMobileMenuOpen(false)} className="font-body text-sm tracking-widest uppercase text-foreground/80 hover:text-primary transition-colors py-2">
                  {link.label}
                </a>)}
              <div className="border-t border-border pt-4 mt-2 flex flex-col gap-3">
                <a href="https://formationheadspapro.com/" target="_blank" rel="noopener noreferrer" className="font-body text-sm tracking-widest uppercase text-primary">
                  Devenir Praticien
                </a>
                <a href="#soins" className="btn-solid-gold rounded-full text-center">
                  Réserver
                </a>
              </div>
            </div>
